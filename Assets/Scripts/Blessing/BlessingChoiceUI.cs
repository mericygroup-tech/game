...
public sealed class BlessingChoiceUI : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, ISelectHandler, IDeselectHandler
{
    ...
    [SerializeField] private bool useIconAsFullCardArtwork;
    ...
    public void Bind(BlessingDefinition blessing, int ownedStack, Action<BlessingDefinition> selectionCallback)
    {
        ...
        if (useIconAsFullCardArtwork && blessing.Icon != null)
        {
            ApplyFullCardArtworkLayout(blessing, ownedStack);
            return;
        }

        SetAllDirectChildrenActive(true);
    ...
    private void ApplyFullCardArtworkLayout(BlessingDefinition blessing, int ownedStack)
    {
        SetFullCardChildrenActive();
        ...
        if (stackText != null)
        {
            ...
            stackText.fontSizeMin = 16f;
            stackText.fontSizeMax = 28f;
            stackText.fontStyle = FontStyles.Bold;
            stackText.color = new Color(1f, 0.93f, 0.58f, 1f);
            stackText.raycastTarget = false;

            RectTransform stackRect = stackText.rectTransform;
            stackRect.anchorMin = new Vector2(0.5f, 0.225f);
            stackRect.anchorMax = new Vector2(0.5f, 0.225f);
            stackRect.pivot = new Vector2(0.5f, 0.5f);
            stackRect.anchoredPosition = Vector2.zero;
            stackRect.sizeDelta = new Vector2(250f, 34f);
            stackText.transform.SetAsLastSibling();
        }
    }

    private void SetFullCardChildrenActive()
    {
        for (int i = 0; i < transform.childCount; i++)
        {
            Transform child = transform.GetChild(i);
            bool keepChild =
                (icon != null && child == icon.transform) ||
                (stackText != null && child == stackText.transform);
            child.gameObject.SetActive(keepChild);
        }
    }

    private void SetAllDirectChildrenActive(bool active)
    {
        for (int i = 0; i < transform.childCount; i++)
